...
import './PerfilCajeroComponent.css';

const PerfilCajeroComponent = () => {
  // Obtener ID del empleado logeado dinámicamente
  const EMPLEADO_ID = getEmpleadoId();
  ...
  const cargarPerfil = async () => {
    try {
      ...
      const response = await obtenerPerfilEmpleado(EMPLEADO_ID);
  ...
  const handleSubirImagen = async () => {
    ...
    try {
      ...
      const response = await subirImagenPerfil(EMPLEADO_ID, nuevaImagen);
      setPerfil(response.data);
      setNuevaImagen(null);
  ...
  const handleEliminarImagen = async () => {
    ...
    try {
      ...
      const response = await eliminarImagenPerfil(EMPLEADO_ID);
      setPerfil(response.data);
      setImagenPreview(null);
      setNuevaImagen(null);
  ...
  const handleGuardarCambios = async () => {
    try {
      ...
      const response = await actualizarPerfilEmpleado(EMPLEADO_ID, datosActualizados);
      setPerfil(response.data);
  ...
  if (loading) {
    return (
      <div className="cajero-perfil-loading">
        <div className="cajero-perfil-spinner"></div>
        <p>Cargando perfil...</p>
      </div>
    );
  }
  ...
  return (
    <div className="cajero-perfil-container">
      {/* Header */}
      <div className="cajero-perfil-header">
        <div className="cajero-perfil-header-content">
          <h1><FiUser /> Perfil</h1>
          <p>Información personal</p>
        </div>
        {!modoEdicion && (
          <button 
            className="cajero-perfil-btn-primary"
      ...
      {/* Alertas */}
      {error && (
        <div className="cajero-perfil-alert error">
          <FiAlertCircle />
          <span>{error}</span>
      ...
      {success && (
        <div className="cajero-perfil-alert success">
          <FiCheck />
          <span>{success}</span>
      ...
      {/* Contenido Principal */}
      <div className="cajero-perfil-content">
        {/* Sección de Imagen */}
        <div className="cajero-perfil-card cajero-perfil-imagen-section">
          <h2>Foto de Perfil</h2>
          
          <div className="cajero-perfil-imagen-container">
            {imagenPreview ? (
              <img 
                src={imagenPreview} 
                alt="Perfil" 
                className="cajero-perfil-imagen"
                onError={(e) => {
              ...
              <div className="cajero-perfil-imagen-placeholder">
                <FiUser size={60} />
              </div>
            )}
          </div>
          ...
          {modoEdicion && (
            <div className="cajero-perfil-imagen-actions">
              <label className="cajero-perfil-btn-secondary">
              ...
              {(imagenPreview || perfil?.imagen) && (
                <button 
                  className="cajero-perfil-btn-danger"
        ...
        <div className="cajero-perfil-card">
          <h2>Información Personal</h2>
          
          <div className="cajero-perfil-grid">
            <div className="cajero-perfil-field">
          ...
          {modoEdicion && (
            <div className="cajero-perfil-field" style={{marginTop: '20px'}}>
          ...
          {modoEdicion && (
            <div className="cajero-perfil-actions">
              <button 
                className="cajero-perfil-btn-primary"
              ...
              <button 
                className="cajero-perfil-btn-secondary"
...
export default PerfilCajeroComponent;
